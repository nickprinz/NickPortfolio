...
export default function LineBetween({toPoint, fromPoint, lineFn = getLine}){
    ...
    return <motion.div className="absolute h-1 bg-black rounded" role="separator"
        transition={{ duration:.8, }} initial={{x:fromPoint.x, y:fromPoint.y, opacity:0, scale: 0, width:0, rotate:rotate}} exit={{x:xPos, y:yPos}} animate={{x:[null,xPos,xPos], y:[null,yPos,yPos], opacity:1, scale:[null, null, 1], width:width, rotate:rotate}} 
        ></motion.div>
}
